...
describe(`specialExpressions`, () => {
  ...
  for (const lits of [new Lits(), new Lits({ debug: true })]) {
    ...
    describe(`if`, () => {
      ...
      describe(`dataType`, () => {
        test(`samples`, () => {
          expect(lits.dataType(`(if true :A "")`)).toEqual(DataType.nonEmptyString)
          expect(lits.dataType(`(if false :A "")`)).toEqual(DataType.emptyString)
          expect(lits.dataType(`(if (@foo) :A "")`)).toEqual(DataType.string)
        })
      })
    })
    ...
    describe(`and`, () => {
      ...
      describe(`dataType`, () => {
        test(`samples`, () => {
          expect(lits.dataType(`(and)`)).toEqual(DataType.true)
          expect(lits.dataType(`(and 1)`)).toEqual(DataType.nonZeroNumber)
          expect(lits.dataType(`(and :foo 0)`)).toEqual(DataType.zero)
          expect(lits.dataType(`(and 1 :foo)`)).toEqual(DataType.nonEmptyString)
          expect(lits.dataType(`(and 1 {})`)).toEqual(DataType.emptyObject)
          expect(lits.dataType(`(and 1 [])`)).toEqual(DataType.emptyArray)

          expect(lits.dataType(`(and)`).is(DataType.truthy)).toBe(true)
          expect(lits.dataType(`(and 1)`).is(DataType.truthy)).toBe(true)
          expect(lits.dataType(`(and :foo 0)`).is(DataType.falsy)).toBe(true)
          expect(lits.dataType(`(and 1 :foo)`).is(DataType.truthy)).toBe(true)
          expect(lits.dataType(`(and 1 {})`).is(DataType.truthy)).toBe(true)
          expect(lits.dataType(`(and 1 [])`).is(DataType.truthy)).toBe(true)
        })
      })
    ...
    describe(`or`, () => {
      ...
      describe(`dataType`, () => {
        test(`samples`, () => {
          expect(lits.dataType(`(or)`)).toEqual(DataType.false)
          expect(lits.dataType(`(or 1)`)).toEqual(DataType.nonZeroNumber)
          expect(lits.dataType(`(or "" 0)`)).toEqual(DataType.zero)
          expect(lits.dataType(`(or 0 "")`)).toEqual(DataType.emptyString)
          expect(lits.dataType(`(or 1 :foo)`)).toEqual(DataType.nonZeroNumber)
          expect(lits.dataType(`(or {} 1)`)).toEqual(DataType.emptyObject)
          expect(lits.dataType(`(or [] 1)`)).toEqual(DataType.emptyArray)

          expect(lits.dataType(`(or)`).is(DataType.falsy)).toBe(true)
          expect(lits.dataType(`(or 1)`).is(DataType.truthy)).toBe(true)
          expect(lits.dataType(`(or "" 0)`).is(DataType.falsy)).toBe(true)
          expect(lits.dataType(`(or 0 "")`).is(DataType.falsy)).toBe(true)
          expect(lits.dataType(`(or 1 :foo)`).is(DataType.truthy)).toBe(true)
          expect(lits.dataType(`(or {} 1)`).is(DataType.truthy)).toBe(true)
          expect(lits.dataType(`(or [] 1)`).is(DataType.truthy)).toBe(true)
        })
      })
