...
describe(`specialExpressions`, () => {
  ...
  describe(`defs`, () => {
    ...
    describe(`undefinedSymbols`, () => {
      test(`samples`, () => {
        expect(getUndefinedSymbolNames(lits.findUndefinedSymbols(`(defs "foo" (+ a b))`))).toEqual(new Set([`a`, `b`]))
  ...
  describe(`def`, () => {
    ...
    describe(`undefinedSymbols`, () => {
      test(`samples`, () => {
        ...
        expect(getUndefinedSymbolNames(lits.findUndefinedSymbols(`(def foo (+ a b)) foo`))).toEqual(new Set([`a`, `b`]))
      })
    })
  })
  ...
  describe(`if`, () => {
    ...
    describe(`dataType`, () => {
      test(`samples`, () => {
        // expect(lits.getDataType(`(if true :A "")`)).toEqual(Type.nonEmptyString)
        // expect(lits.getDataType(`(if false :A "")`)).toEqual(Type.emptyString)
        // expect(lits.getDataType(`(if (@foo) :A "")`)).toEqual(Type.string)
      })
    })
  })
  ...
  describe(`if-not`, () => {
    ...
    describe(`undefinedSymbols`, () => {
      test(`samples`, () => {
        expect(getUndefinedSymbolNames(lits.findUndefinedSymbols(`(if-not (> a b) a b)`))).toEqual(new Set([`a`, `b`]))
  ...
  describe(`and`, () => {
    ...
    describe(`dataType`, () => {
      test(`samples`, () => {
        // expect(lits.getDataType(`(and)`)).toEqual(Type.true)
        // expect(lits.getDataType(`(and 1)`)).toEqual(Type.positiveInteger)
        // expect(lits.getDataType(`(and :foo 0)`)).toEqual(Type.zero)
        // expect(lits.getDataType(`(and 1 :foo)`)).toEqual(Type.nonEmptyString)
        // expect(lits.getDataType(`(and 1 {})`)).toEqual(Type.emptyObject)
        // expect(lits.getDataType(`(and 1 [])`)).toEqual(Type.emptyArray)
        // expect(lits.getDataType(`(and)`).is(Type.truthy)).toBe(true)
        // expect(lits.getDataType(`(and 1)`).is(Type.truthy)).toBe(true)
        // expect(lits.getDataType(`(and :foo 0)`).is(Type.falsy)).toBe(true)
        // expect(lits.getDataType(`(and 1 :foo)`).is(Type.truthy)).toBe(true)
        // expect(lits.getDataType(`(and 1 {})`).is(Type.truthy)).toBe(true)
        // expect(lits.getDataType(`(and 1 [])`).is(Type.truthy)).toBe(true)
      })
    })
  ...
  describe(`or`, () => {
    ...
    describe(`dataType`, () => {
      test(`samples`, () => {
        // expect(lits.getDataType(`(or)`)).toEqual(Type.false)
        // expect(lits.getDataType(`(or 1)`)).toEqual(Type.positiveInteger)
        // expect(lits.getDataType(`(or "" 0)`)).toEqual(Type.zero)
        // expect(lits.getDataType(`(or 0 "")`)).toEqual(Type.emptyString)
        // expect(lits.getDataType(`(or 1.1 :foo)`)).toEqual(Type.positiveNonInteger)
        // expect(lits.getDataType(`(or {} 1)`)).toEqual(Type.emptyObject)
        // expect(lits.getDataType(`(or [] 1)`)).toEqual(Type.emptyArray)
        // expect(lits.getDataType(`(or)`).is(Type.falsy)).toBe(true)
        // expect(lits.getDataType(`(or 1)`).is(Type.truthy)).toBe(true)
        // expect(lits.getDataType(`(or "" 0)`).is(Type.falsy)).toBe(true)
        // expect(lits.getDataType(`(or 0 "")`).is(Type.falsy)).toBe(true)
        // expect(lits.getDataType(`(or 1 :foo)`).is(Type.truthy)).toBe(true)
        // expect(lits.getDataType(`(or {} 1)`).is(Type.truthy)).toBe(true)
        // expect(lits.getDataType(`(or [] 1)`).is(Type.truthy)).toBe(true)
      })
    })
  ...
  describe(`defns`, () => {
    ...
    describe(`undefinedSymbols`, () => {
      test(`samples`, () => {
        ...
        expect(getUndefinedSymbolNames(lits.findUndefinedSymbols(`(defns :foo [a b] (str a b c)) (foo x y)`))).toEqual(
          new Set([`c`, `x`, `y`, `foo`]),
        )
        expect(
  ...
  describe(`try`, () => {
    ...
    describe(`undefinedSymbols`, () => {
      test(`samples`, () => {
        expect(getUndefinedSymbolNames(lits.findUndefinedSymbols(`(try (/ a b) (catch error (str error x)))`))).toEqual(
          new Set([`a`, `b`, `x`]),
        )
      })
    })
